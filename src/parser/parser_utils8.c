...
/*   Updated: 2025/09/25 19:27:20 by ekakhmad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_redir	*init_redir(t_token *cur)
{
	...
	if (!redir)
		return (NULL);
       // Safeguard: force TOKEN_APPEND for '>>'
       if (cur->type == TOKEN_HEREDOC && cur->fragments && cur->fragments->text && ft_strncmp(cur->fragments->text, ">>", 3) == 0)
